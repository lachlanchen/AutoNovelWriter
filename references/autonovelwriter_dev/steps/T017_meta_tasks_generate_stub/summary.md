...
## Next
1. Implement true `ROUND`/`FOREACH_TASK` execution semantics (persisted loop counters + resume pointers) so `meta_tasks_generate` can run once per round, not per task.
